@tool
class_name ScrollGridContainer
extends Container
...
@export var line_item_count : int = 4: set = set_line_item_count
@export var line_min_size : float = 0.0: set = set_line_min_size
@export var line_max_size : float = -1.0: set = set_line_max_size
...
@export var background_offset : Vector2 = Vector2(): set = set_bg_offset
@export var background_shrink : Vector2 = Vector2(): set = set_bg_shrink
...
func set_bg_offset(_offset):
    background_offset = _offset
    if is_inside_tree():
        _fix_bg()
    queue_redraw()

func set_bg_shrink(_val):
    background_shrink = _val
    if is_inside_tree():
        _fix_bg()
    queue_redraw()
...
func set_line_item_count(_count):
    _count = max(1, _count)
    line_item_count = _count
    queue_sort()
    queue_redraw()

func set_line_min_size(_size):
    line_min_size = _size
    queue_sort()
    queue_redraw()

func set_line_max_size(_size):
    line_max_size = _size
    queue_sort()
    queue_redraw()
...
func _calculate_bg_rect():
    ...
    if t:
        ...
        var pos = Vector2(-left, -top) + background_offset + background_shrink
        var _bg_extra_size = Vector2()
        _bg_extra_size.x = left + right
        _bg_extra_size.y = top + bottom
        var _bg_rect_size = size + _bg_extra_size - background_shrink*2.0
...
func _fix_bg():
    ...
    if !visible or !background_texture:
        return
...
func _do_reflow(visible_scroll : bool):
    ...
    for _child in get_children():
        if _child == scrollbar_v or _child == scrollbar_h or _child.is_set_as_top_level() or !_child.visible:
            continue
        if _child is Control or _child is Node2D:
            if lines.back().size() >= line_item_count:
                lines.push_back([])
            lines.back().push_back(_child)
    
    for line in lines:
        var ms_cache = {}
        var largest_size = Vector2()
       # var total_size = Vector2()
        for child in line:
            var ms = child.get_combined_minimum_size()
            ms_cache[child] = ms
            largest_size.x = max(largest_size.x, ms.x)
            largest_size.y = max(largest_size.y, ms.y)
            #total_size += ms
        
        var line_size : float = largest_size.y if vertical else largest_size.x
        
        var cell_size : Vector2
        var cell_advance : float
        var spacing_count = line_item_count-1
        if vertical:
            cell_size.x = (rect.size.x - spacing*spacing_count) / line_item_count
            cell_size.y = max(line_min_size, largest_size.y)
            if line_max_size >= 0:
                cell_size.y = min(cell_size.y, line_max_size)
        else:
            cell_size.x = max(line_min_size, largest_size.x)
            if line_max_size >= 0:
                cell_size.x = min(cell_size.x, line_max_size)
            cell_size.y = (rect.size.y - spacing*spacing_count) / line_item_count
        
        for _child in line:
            if _child is Control:
                var child : Control = _child
                var remaining = Rect2(rect.position + cursor, cell_size)
                fit_child_in_rect(child, remaining)
                if vertical:
                    cursor.x += cell_size.x + spacing
                else:
                    cursor.y += cell_size.y + spacing
